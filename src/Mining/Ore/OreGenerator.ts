...
export class OreGenerator {
  ...
  /**
   * Generates an Island 4 ore type
   */
  private generateIsland4Ore(currentDepth: number, luck: number): string {
    const availableOres = Object.values(ISLAND4_ORE_DATABASE).filter(
      ore => currentDepth >= ore.firstDepth
    );

    if (availableOres.length === 0) {
      return ISLAND4_ORE_TYPE.FROSTBRICK;
    }
  ...
  getOreHealth(oreType: string, currentDepth: number, worldId: string = 'island1'): number {
    ...
    if (worldId === 'island2') {
      ...
      if (oreType in ISLAND4_ORE_DATABASE) {
        const oreData = ISLAND4_ORE_DATABASE[oreType as ISLAND4_ORE_TYPE];
        return calculateOreHealth(oreData, currentDepth);
      }
